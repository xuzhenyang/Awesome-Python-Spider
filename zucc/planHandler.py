...
def parseTable(str):
    ...
    list = []
    #读取每一行记录
    for tag in soup.find('table', {'id':'DBGrid'}).find_all('tr')[1:]:
        #使用keys列表来遍历字典，因为直接并行遍历字典，顺序会乱
        keys = ['kcdm', 'kcmc',  'xf', 'zxs', 'kcxz', 'kclb', 'jyxdxq', 'kcxx', 'fxbs', 'zyfx', 'sfdxq']
        dict = {'kcdm':'', 'kcmc':'', 'xf':'', 'zxs':'', 'kcxz':'', 'kclb':'', 'jyxdxq':'', 'kcxx':'', 'fxbs':'', 'zyfx':'', 'sfdxq':''}
        #zip并行遍历并赋值
...
def main():
    #do something
    str = readFile('plan.html')
    list = parseTable(str)
    json.dump(list, open('plan.txt', 'w'))
